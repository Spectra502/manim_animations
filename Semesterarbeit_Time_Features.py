 ...
 * @modify date 2024-10-03 11:43:36
 * @desc [description]
"""
...
class KurtosisVisualization(Scene):
    def construct(self):
        ...
        mesokurtic_label = Tex("Mesokurtic (Normal)").next_to(axes, UP)

        # Leptokurtic (Heavy Tails)
        leptokurtic_curve = axes.plot(lambda x: np.exp(-0.5 * x**2), color=GREEN, x_range=[-4, 4])
        leptokurtic_label = Tex("Leptokurtic (Heavy Tails)").next_to(axes, UP)

        # Platykurtic (Light Tails)
        platykurtic_curve = axes.plot(lambda x: np.exp(-x**4), color=RED, x_range=[-4, 4])
        platykurtic_label = Tex("Platykurtic (Light Tails)").next_to(axes, UP)
        ...
        self.wait(1)
        self.play(FadeOut(mesokurtic_curve, mesokurtic_label))
